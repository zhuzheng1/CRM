...
<% 
	...
	//根据pMap准备pMap中的key集合
	Set<String> set = (Set<String>)pMap.keySet();
	
	//准备:前面正常阶段和后面丢失阶段的分界点下标
	int point = 0;
	for(int i=0; i<dvList.size(); i++) {
		//取得每一个字典值
		DicValue dc = dvList.get(i);
		
		//从dv中取得value
		String stage = dc.getValue();
	...
	function changeStage(stage,i) {
		$.ajax({
			...
			data : {
				...
				"expectedDate" : "${t.expectedDate}"//同上
			},
			type : "post",
			dataType : "json",
			success : function(data) {
				if(data.flag) {
					//改变阶段成功后，需要在详细页上局部刷新，刷新阶段，可能性，修改人，修改时间
...
// 		alert(point1);
		...
		if(currentPossibility == "0") {
			...
			//遍历后两个
			for(var i=point1; i<<%=dvList.size()%>; i++) {
				//如果是当前阶段
				if(i == index) {
					//红叉
					//先移除掉原有的样式
		...
		}else {  //如果当前阶段可能性不为0，前七个可能是绿圈，绿色标记，黑圈，后两个一定是黑叉
			//遍历前七个  绿圈，绿色标记，黑圈
			for(var i=0; i<point1; i++) {
				if(i == index) {//当前阶段
					//绿色标记
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			if("0".equals(currentPossibility)) {
				for(int i=0; i<dvList.size(); i++) {
					//取得每一个遍历出来的阶段，然后根据每一个遍历出来的阶段取其可能性
			...
			<%		
					...
					}else {//如果遍历出来的阶段的可能性不为0，说明是前7个，一定是黑圈
						//黑圈
						%>
						<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" 
			...
			}else {//前七个有可能是绿圈，绿色标记，黑圈，后两个一定是黑叉
				//准备当前阶段的下标
				int index = 0;
				for(int i=0; i<dvList.size(); i++) {
					DicValue dv = dvList.get(i);
				...
				for(int i=0; i<dvList.size(); i++) {
					...
					if("0".equals(listPossibility)) {
						...
						----------- --
						<%
					}else {//如果遍历出来的阶段的可能性不为0，说明前七个阶段  绿圈，绿色标记，黑圈
						//如果是当前阶段
						if(i == index) {
							//绿色标记
							%>
